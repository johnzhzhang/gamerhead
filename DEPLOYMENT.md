...
请选择操作:
  ...
  3) 管理服务配置 (用户 / 管理员 / Autopilot)
```

> 模式 3 是所有「不重新部署就能改」的配置入口：可登录用户、Admin 管理员名单、
> Autopilot 开关。会自动适配 IAP / GIS / 固定密码三种登录方式。
>
> Autopilot 是模式 3 的第 3 项，**默认关闭**，不开启的部署与加入该功能之前完全一致。
> 直接输入 `4` 仍可跳到 Autopilot（旧的独立模式号保留为别名）。
>
> 输入无法识别的选项会直接报错退出，不会落到「全新部署」—— 那条路径会改动认证
> 配置，打错一个字不该有这种后果。
...
选 `y` 时会在部署完成后自动配置存储桶 CORS 与常驻实例。之后随时可用模式 3 → Autopilot 开关。
...
**升级老版本就是模式 2，不需要别的动作。** 它只重建镜像，其余一切照旧：环境变量、
登录方式、IAP、`min-instances`、IAM 全部保留（脚本刻意不传 `--iap` / `--no-iap`，
也从不使用 `--env-vars-file`）。

```bash
git pull
./deploy.sh --lang=zh <<< $'2\ny\n你的服务名\n你的区域\ny\n'
# 或者直接 ./deploy.sh 然后选 2
```
...
升级完就停在这里的部署，行为与升级前**完全一致**：`/api/autopilot/*` 返回 404，
界面上不出现 Autopilot 标签 —— 因为代码在运行时检查 `AUTOPILOT_ENABLED`，
不会因为镜像里有这段代码就自动启用。

### 之后再开 Autopilot

```bash
./deploy.sh        # → 3（管理服务配置）→ 3（Autopilot）→ 1（启用）
```

这一步与代码升级刻意分开，因为启用要做的事不该由一次代码部署顺手完成：

| 它改了什么 | 为什么不能省 |
|-----------|------------|
| `AUTOPILOT_ENABLED` 与批量限额 | 不写这些，所有 Autopilot 路由都还是 404 |
| **存储桶 CORS** | 素材由浏览器直传 Cloud Storage。没有 CORS 规则，上传会以浏览器端的模糊错误失败，服务端看不到任何记录。应用自己补不了：运行时服务账号只有 `roles/storage.objectAdmin`，它**不含** `storage.buckets.update` |
| `--min-instances=1 --no-cpu-throttling` | Cloud Run 在请求之外限制 CPU，没有常驻实例时批次只能在页面打开时推进。这是唯一空闲也计费的配置，用选项 2 可随时回退 |

老部署没有任何东西会挡住这一步。Firestore（Datastore 模式）无模式，新的 `AutopilotJob`
kind 不需要迁移也不需要索引，升级前建的数据库直接就能写第一条作业。

### 升级路径的实测结论

用 Autopilot 之前的提交 + **老版本 deploy.sh** 全新部署一套服务，再升级并开启，逐项核对：

| 阶段 | 观察到的 |
|------|---------|
| 升级前 | 5 个环境变量 · 桶无 CORS · `min-instances` 未设 · 所有 `/api/autopilot/*` → 404 |
| 模式 2 之后 | 仍是 5 个变量，没凭空多出 `AUTOPILOT_*` · CORS 仍无 · IAP 仍开 · 无 `allUsers` · Autopilot 路由仍 404 · `/api/health`、`/api/me` 正常 · 新前端已就位但显示「未启用」兜底 |
| 启用之后 | 原 5 个变量保留、新增 4 个 · CORS 建好且含两种 Cloud Run 域名 · `min-instances=1`、CPU 常开 · IAP 未被动过 |
| 在这套升级来的服务上实测 | 签名直传返回 200 且对象落桶 · 图片上传 200 · 建作业成功并停在确认闸门 · 连续 3 次 `tick` 都不推进 · 作业列表能从升级前就存在的数据库里读出 |
| 关闭之后 | 回到 5 个变量 · `min-instances` 未设 · CPU 恢复节流 · 路由 404 · 已产出的作业与视频仍在 Datastore 和桶里 |

> ⚠️ **不要用重跑模式 1 的方式「升级」。** 模式 1 会改写认证配置，其中选项 1 / 2
> 会传 `--allow-unauthenticated`，等于让流量绕过 IAP 直达容器。
...
## 七、配置 Autopilot 批量成片（模式 3 → 第 3 项）

Autopilot 让用户填一次表单、确认一次主播形象，然后一次拿到多个成片。**默认关闭**。

```bash
./deploy.sh
# 选择 3（管理服务配置）→ 再选 3（Autopilot）
# 也可以直接输入 4，会跳到同一处
```
...
> ⚠️ **这里只改配置，不部署代码。** 如果开启后界面上看不到 Autopilot 标签，说明当前运行的镜像还没有这个功能，先执行模式 2 更新代码。
...
常驻实例是唯一会产生固定费用的配置项，用模式 3 → Autopilot → 选项 2 可随时关掉。
...
**如果启用了 Autopilot（模式 3 → Autopilot）还要确认：**
...
- [ ] 不需要时用模式 3 → Autopilot → 选项 2 关闭，min-instances 回落为 0

...
const KEY_SIMILARITY = 0.10;   // 4/4 test clips keyed 100% clean at this value;
                               // 0.20 started eating the subject
const KEY_BLEND = 0.08;
/** Green is "dominant" when it leads both other channels by this much. */
const GREEN_DOMINANCE = 40;
...
/**
 * Sample the four corners of a frame to find the key colour.
 *
 * Corners are used because the subject is framed centrally; this is the same
 * measurement the avatar validation gate applies, so a clip that passes there
 * keys predictably here.
 */
const sampleBackgroundColour = async (videoPath, { at = 0.5 } = {}) => {
    const W = 160;
    const H = 90;
    ...
    const patch = (x0, y0) => {
        const out = [];
        for (let dy = 0; dy < 12; dy += 1) for (let dx = 0; dx < 12; dx += 1) out.push(px(x0 + dx, y0 + dy));
        return out;
    };
    const bg = [
        ...patch(0, 0), ...patch(W - 12, 0), ...patch(0, H - 12), ...patch(W - 12, H - 12),
    ];

    const r = mean(bg.map((p) => p[0]));
    const g = mean(bg.map((p) => p[1]));
    const b = mean(bg.map((p) => p[2]));
    const spread = Math.max(stdev(bg.map((p) => p[0])), stdev(bg.map((p) => p[1])), stdev(bg.map((p) => p[2])));
    ...
    return {
        ...
        b: Math.round(b),
        spread,
        greenDominant: g > r + GREEN_DOMINANCE && g > b + GREEN_DOMINANCE,
        hex: `0x${[r, g, b].map((v) => Math.round(v).toString(16).padStart(2, '0')).join('').toUpperCase()}`,
    };
};
 ...
 */
const buildFilterGraph = ({ geo, duration, gameplayHasAudio, streamerHasAudio, volumes, keyColour = null }) => {
    ...
    if (streamer.fit === 'cover') {
        ...
        chains.push(
            ...
            + `chromakey=${keyColour}:${KEY_SIMILARITY}:${KEY_BLEND},`
            + `format=rgba[st]`
        );
        chains.push(`[bg][st]overlay=x=${r0(streamer.x)}:y=${r0(streamer.y)}:eof_action=pass[vout]`);
    } else {
...
const composeVideo = async ({
    ...
    if (removeBackground && geo.layout === 'classic-pip') {
        keySample = await sampleBackgroundColour(streamerPath, {
            at: Math.min(0.5, Math.max(0, streamerMeta.duration / 2)),
        });
        if (!keySample.greenDominant) {
            throw new Error(
                `Background removal needs a green-screen streamer clip, but its corners are `
                + `rgb(${keySample.r},${keySample.g},${keySample.b}) — not green. `
                + 'Regenerate the avatar with background removal enabled.'
            );
        }
        keyColour = keySample.hex;
    }

...
> 状态:**P1–P6 已全部实施并实测通过**(本文档保留为设计记录)
...
- [15. 实施分期](#15-实施分期)
- [16. 风险登记](#16-风险登记)
- [17. 决策记录](#17-决策记录已拍板-2026-09-03)
- [18. 原始待确认事项(留档)](#18-原始待确认事项留档)
...
| 模式 | 脚本 | 头像 | 确认闸门形态 | 用途 |
|---|---|---|---|---|
| `vary-script`(**已选定**) | 10 份不同(升温 / 换切入角度) | 锁定 1 个 | **确认 1 张图** | 测文案,主播形象统一 |
...
| `AUTOPILOT_VEO_CLIP_BUDGET` | `ceil(总片段×25%)`,最低 4 | 单作业允许走 Veo 的片段数上限,封顶按量付费敞口 |
...
2. **Veo 预算封顶**(`AUTOPILOT_VEO_CLIP_BUDGET`)—— 已决定保留 Veo 兜底(决策 ③),故改为限量:救援个别 `content_blocked` 照常,但"Omni 全线崩 → 40 个 Veo"被封顶
3. `AUTOPILOT_MAX_CLIPS_PER_JOB` 熔断
4. 确认闸门上**显式显示"确认后将生成 N 个片段;其中最多 M 个可能使用 Veo(按量付费)"**,让花钱这一步有明确知情
...
| 阶段 | 状态 | 实测要点 |
|---|---|---|
| P1 | ✅ | `lib/compose.js`;16 项测试含 7 项像素级渲染验证 |
| P2 | ✅ | 250MB 直传成功;生产实测 40MB POST 返回 413,证实 32 MiB 硬限制 |
| P3 | ✅ | 闸门 10 次 tick + 5 次轮询零推进;`kill -9` 后从断点续跑 |
| P4 | ✅ | 修掉并发锁死与丢失更新;12 片段 608s → 184s(3.3x),进度零回退 |
| P5 | ✅ | CDP 驱动真实 Chrome 端到端:闸门静置 25s 不开工,确认后才生产 |
| P6 | ✅ | 模式 4 启用/关闭/查看均验证;全新部署带 Autopilot 验证后已拆除 |

实施中发现并修正的设计缺陷:

1. **并发按 jobId 加锁** → 同一作业只允许一个 worker,批次实际串行。改为按动作粒度占用。
2. **丢失更新** → 多 worker 整体覆写实体,进度回退、已失败变体复活。改为事务化读改写。
3. **CORS 只覆盖一种域名** → Cloud Run 同时响应两种 host 形式,用户从另一个进来直传就失败。改为两种都写入。
4. **模式 4 只改配置不部署代码** → 老镜像上开启后看不到入口。加了明确提示指向模式 2。
...
## 17. 决策记录(已拍板 2026-09-03)

| # | 决策项 | 结论 | 说明 |
|---|---|---|---|
| ① | 变体语义 | **(A)** 一个 gameplay 出 N 个不同成片 | 变体来自脚本差异 |
| ② | 确认闸门粒度 | **(A)** 只做 `vary-script` | 确认 **1 张**主播图 → N 个文案变体共用。图廊多选留作 P5 后增量 |
| ③ | Veo 兜底 | **(B) 保留** | ⚠️ 与原建议相反,已按此调整成本护栏,见下 |
| ④ | 常驻实例 | **(A)** 接受 `min-instances=1` + CPU 常开 | 关掉页面阶段 B 继续跑 |

### ③ 保留 Veo 带来的成本敞口与应对

保留 Veo 的收益明确:Omni 的 RAI 输出过滤对照片级真人是非确定性拦截,Veo 支持 `personGeneration: allow_adult`,是这类失败唯一的救援手段。

但批量场景放大了风险:**若 Omni 整体不可用,40 个片段会全部落到按量付费的 Veo**,账单可能远超预期。因此在原护栏之上增加一道:

| 护栏 | 机制 |
|---|---|
| **Veo 预算(新增)** | `AUTOPILOT_VEO_CLIP_BUDGET` —— 单作业允许走 Veo 的片段数上限,默认 `ceil(总片段数 × 25%)` 且不低于 4。超预算后剩余片段**不再落 Veo**,该变体直接失败 |
| 片段总数熔断 | `AUTOPILOT_MAX_CLIPS_PER_JOB`(默认 60) |
| 闸门成本提示 | 确认闸门显示"确认后将生成 N 个片段;其中最多 M 个可能使用 Veo(按量付费)" |

这样保住了 Veo 的救援价值(个别 `content_blocked` 照常兜住),同时把"Omni 全线崩 → 40 个 Veo"的尾部风险封顶。
...
- **(A) 已选** —— 一个 gameplay 出 10 个不同成片(变体来自脚本/头像差异)
...
- **(A) 已选** —— 只做 `vary-script`:确认 **1 张**主播图 → 10 个文案变体共用。人工负担最小,P5 也最快落地
- (B) —— 同时做候选图廊:一次生成 N+2 张,**多选**勾中要用的,支持 `vary-avatar` / `vary-both`
- (C) —— 除头像外,**脚本也要确认**(与"脚本不用每步操作"的原始需求冲突,列出仅供排除)

### ③ Veo 兜底

- (A) 原建议 —— Autopilot 下默认**关闭** Veo:省钱,失败即失败
- **(B) 已选** —— 保留 Veo:成功率优先,配 Veo 预算封顶尾部风险

### ④ 常驻实例

- **(A) 已选** —— 接受 `min-instances=1` + CPU 常开,换取"关掉页面继续跑"

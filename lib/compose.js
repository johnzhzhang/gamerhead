...
//
// Applied before scaling, so the cost is a pixel of real subject at source
// resolution — invisible once the streamer is scaled down to picture-in-picture.
// One pass is enough: with the key already applied at source resolution, a
// composite over white showed only 17 residual dark pixels in the whole PiP box.
const CUTOUT_EDGE_CHOKE = Math.max(0, Number(process.env.CUTOUT_EDGE_CHOKE ?? 1));
...
const buildFilterGraph = ({ geo, duration, gameplayHasAudio, streamerHasAudio, volumes, keyed = false }) => {
    ...
    if (streamer.fit === 'cover') {
        ...
        // Key at the source resolution, choke the edge, then scale. Keying after
        // scaling merges the dark contour into the subject's own dark pixels, which

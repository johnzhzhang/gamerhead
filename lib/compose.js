...
// Edge choke, in pixels of alpha erosion applied at the source resolution.
//
// Needed because the model draws a dark contour between the subject and the green
// field. Measured on real output, that band runs 4-8px wide with a green ratio of
// 0.75-1.16 — it is neutral or warm dark, not green at all, so no colour test can
// separate it from the subject's own dark hair. Only a spatial operation can:
// shrink the kept region so the band goes with the background.
//
// Applied at the picture-in-picture size, after despill has taken care of the
// green end of the transition band. Measured on a real moving frame: the band held
// 121 greenish pixels inside the PiP box, despill leaves 0 at no extra cost, and
// three erosion passes take the neutral-dark end from 39 to 9.
const CUTOUT_EDGE_CHOKE = Math.max(0, Number(process.env.CUTOUT_EDGE_CHOKE ?? 3));
...
/** Share the pixel test between the file and buffer paths so they cannot drift. */
const greenFractionOf = (raw) => {
    const total = SAMPLE_W * SAMPLE_H;
    if (raw.length < total * 3) throw new Error('Could not decode a frame to measure');
    let green = 0;
...
const measureGreenScreen = async (videoPath, { at = 0.5 } = {}) => greenFractionOf(
    await runBinary('ffmpeg', [
        '-v', 'error', '-ss', String(at), '-i', videoPath, '-frames:v', '1',
        '-vf', `scale=${SAMPLE_W}:${SAMPLE_H}`, '-f', 'rawvideo', '-pix_fmt', 'rgb24', '-',
    ])
);

/**
 * Same measurement for an image held in memory.
 *
 * The avatar validation gate runs before anything is written to storage, so it
 * checks the bytes the model returned rather than a file on disk.
 */
const measureGreenScreenImage = async (buffer) => greenFractionOf(
    await runBinary('ffmpeg', [
        '-v', 'error', '-f', 'image2pipe', '-i', 'pipe:0',
        '-frames:v', '1', '-vf', `scale=${SAMPLE_W}:${SAMPLE_H}`,
...
const runBinary = (bin, args, { stdin = null } = {}) => new Promise((resolve, reject) => {
    const p = spawn(bin, args);
    if (stdin) {
        p.stdin.on('error', () => {});   // ffmpeg may close early once it has a frame
        p.stdin.end(stdin);
    }
    const out = [];
    let err = '';
...
const buildFilterGraph = ({ geo, duration, gameplayHasAudio, streamerHasAudio, volumes, keyed = false }) => {
    ...
    if (streamer.fit === 'cover') {
        ...
        // Key and choke at the picture-in-picture size. Erosion does not care
        // whether a dark pixel is the model's contour or the subject's own hair; it
        // just trims the silhouette, so it works at this size too — and doing it
        // here rather than at the video's native resolution is ~4x less work for
        // the same result. Verified with a 6px contour: 3 passes leave zero
        // residual dark pixels.
        const choke = CUTOUT_EDGE_CHOKE > 0
            ? `,split[ck][cm];[cm]alphaextract,${Array(CUTOUT_EDGE_CHOKE).fill('erosion').join(',')}[ca];`
              + `[ck][ca]alphamerge`
            : '';
        chains.push(
            ...
            + `*gt(g(X\\,Y)\\,${KEY_GREEN_FLOOR}),0,255)'`
            + `,despill=type=green:mix=1.0:expand=0${choke}[st]`
        );
        chains.push(`[bg][st]overlay=x=${r0(streamer.x)}:y=${r0(streamer.y)}:eof_action=pass[vout]`);
    } else {
        chains.push(

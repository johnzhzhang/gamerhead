 ...
 *
 * `awaiting_avatar` resolves to WAIT_APPROVAL, never to work. The executor,
 * the poll-driven tick and the resume sweep all consult this function, so the
 * human gate is enforced in one place for every caller.
 */
export const nextAction = (job) => {
    if (!job || isTerminal(job)) return { type: ACTION.NOTHING };

    if (job.status === JOB_STATUS.CREATED) {
        return job.avatar?.candidates?.length
    ...
    for (const v of job.variants) {
        if (v.stage === VARIANT_STAGE.PENDING) {
            return { type: ACTION.GENERATE_SCRIPT, variantIdx: v.idx };
        }
    }
    for (const v of job.variants) {
        if (v.stage === VARIANT_STAGE.CLIPS) {
            const nextClip = v.clipUris.findIndex((u) => !u);
            if (nextClip !== -1) {
                return { type: ACTION.GENERATE_CLIP, variantIdx: v.idx, clipIdx: nextClip };
            }
        }
    }
    for (const v of job.variants) {
        if (v.stage === VARIANT_STAGE.COMPOSE) {
            return { type: ACTION.COMPOSE, variantIdx: v.idx };
        }
    }
    const settled = job.variants.every(
        (v) => v.stage === VARIANT_STAGE.DONE || v.stage === VARIANT_STAGE.FAILED
    );
    return settled ? { type: ACTION.FINALISE } : { type: ACTION.NOTHING };
};
